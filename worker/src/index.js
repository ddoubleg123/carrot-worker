...
function logStartupInfo() {
  ...
  if (!process.env.FIREBASE_STORAGE_BUCKET) {
    console.error('[FATAL] FIREBASE_STORAGE_BUCKET environment variable is required');
    process.exit(1);
  }
  
  if (!process.env.GOOGLE_APPLICATION_CREDENTIALS_JSON) {
    console.error('[FATAL] GOOGLE_APPLICATION_CREDENTIALS_JSON environment variable is required');
    process.exit(1);
  }
